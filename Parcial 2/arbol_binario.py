...
class Arbol(object):
    ...
    def dato_mal_cargado(self):
        ...
        if(pos):
            ...
            self.insertar_nodo(nuevo_nombre, datos)
    #FIN EJERCICIO 5
    ...
    def info_dino(self,  dinosaurio):
        """muestra toda la info de los t-rex que hay en la isla"""
        if(self.info is not None):
            if(self.info == dinosaurio):
                print(self.datos)
            if(self.izq is not None):
                self.izq.info_dino(dinosaurio)
            if(self.der is not None):
                self.der.info_dino(dinosaurio)

    def recargar_dato(self):
        """modifica usuario mal cargado, cambia nombre"""
        buscado = input('ingrese lo que desa buscar ')
        self.busqueda_proximidad(buscado)
        buscado_modificar = input('ingrese el nombre que desea modificar ')
    ...
    def ubicacion(self,  dinosaurio):
        """muestra la ubicacion de los raptores que hay en la isla"""
        if(self.info is not None):
            if(self.info == dinosaurio):
